...
//	Log(LOG_SCHEDINTF, "[setTimer] New timer set to %lu.%u\n", currentTimer.sec, currentTimer.msec);
	// set OS-timer
}

Time *getTimer () {
//	Log(LOG_SCHEDINTF, "[getTimer] returning timer of %lu.%u\n", currentTimer.sec, currentTimer.msec);
	return &currentTimer;
...
void executeTasks () {
	...
	while(toExecute != NULL) {
		...
		Log(LOG_SCHEDINTF, "[executeTasks] Executing task %x\n", toExecute->identifier);
